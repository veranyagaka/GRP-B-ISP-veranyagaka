...
class PneumoniaCNN(nn.Module):
    def __init__(self, in_channels=1):
        ...
        # ❗ Removed global average pooling (it destroys spatial detail)
        self.flatten = nn.Flatten()

        # New fully connected head (larger input → must detect during forward)
        self.fc1 = nn.Linear(256 * 14 * 14, 512)
        self.dropout = nn.Dropout(0.5)
        self.fc2 = nn.Linear(512, 1)  # 1 output → SIGMOID
...
# load_state_dict(strict=False) allows new head if different shape
...
def preprocess_image(image_path, target_size=(224, 224)):
    ...
    # Normalize similarly to common X-ray preprocessing
    img = img.astype("float32") / 255.0
    mean, std = 0.5, 0.25
    img = (img - mean) / std

    # OOD / invalid image detection
    if np.std(img) < 0.05:  # nearly uniform image
        return "INVALID"
